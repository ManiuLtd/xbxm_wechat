  ...
  <div>
    ...
    <el-form :model="addCouponForm" label-width="110px" class="addCouponForm" :rules="addCouponFormRules" ref="addCouponForm">
      ...
      <el-form-item label="领取有效期" prop="sendTime">
        <el-date-picker :picker-options="pickerOptions1" v-model="addCouponForm.sendTime" type="datetimerange" range-separator="—"
          start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="使用有效期" prop="validTime">
        <el-date-picker v-model="addCouponForm.validTime" :picker-options="pickerOptions2" type="datetimerange" range-separator="—"
          start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']">
        </el-date-picker>
      </el-form-item>
    ...
    <el-dialog title="商户选择" :visible.sync="dialogTableVisible" :modal="false" width="500px" height="800px" class="m-merchant-dialog">
      <div class="u-search-input">
        <el-input v-model="serch_merchant_input" placeholder="商户/店面名称"></el-input>
        <el-button type="primary" @click="search_merchant">搜索</el-button>
      </div>
      <div>
        <ul class="m-merchant-list">
          <li v-for="(item,index) in merchant" :key="index">
            <div class="u-overflow-content">
      ...
      <div class="u-page">
        <el-pagination background @current-change="page_change" layout="prev, pager, next" :current-page="param.pageNum" :total="totalPage*10">
        </el-pagination>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    getDetailData() {
      ...
      ApiSetting.couponInfo.data = param;
      this.$http(ApiSetting.couponInfo).then(msg => {
        if (!msg || msg.data.errCode !== 0 || msg === undefined || msg === null)
          return;
        this.dealData(msg.data.data);
      });
    },
    dealData(data) {
      this.addCouponForm.dataFlag = data.dataFlag;
      this.addCouponForm.couponName = data.couponName;
      this.addCouponForm.couponPrice = data.couponPrice;
      this.addCouponForm.couponMoney = Number(data.couponMoney);
      this.addCouponForm.remark = data.remark;
      this.addCouponForm.sendNum = data.sendNum;
      this.addCouponForm.sendTime = [];
      this.addCouponForm.validTime = [];
      this.addCouponForm.sendTime.push(data.sendStartTime * 1000);
      this.addCouponForm.sendTime.push(data.sendEndTime * 1000);
      this.addCouponForm.validTime.push(data.validStartTime * 1000);
      this.addCouponForm.validTime.push(data.validEndTime * 1000);
      // 商场数据
      if (data.shopId == 0) {
        this.addCouponForm.shopId = "全场通用";
        this.isshowMarket = false;
      } else {
        this.addCouponForm.shopId = "按商场可用";
        this.isshowMarket = true;
        data.shopId.map(shopitem => {
          this.market.map(marketItem => {
            if (shopitem.marketUniqueCode == marketItem.marketUniqueCode) {
              marketItem.state = true;
            }
          });
        });
      }
      // 商户数据
      if (data.couponClass == 0) {
        this.addCouponForm.supplierId = "全场通用";
      } else {
        this.addCouponForm.supplierId = "按商户可用";
        this.isshowMerchant = true;
        this.afterMerchant = data.supplierId;
      }
      // 限制条件
      if (data.couponType == 0) {
        this.addCouponForm.couponType = "满";
        this.addCouponForm.minimumMoney = data.minimumMoney;
      } else {
        this.addCouponForm.couponType = "不限制";
      }
    },
    ...
    add_dealData() {
      ...
      // 验证时间
      if (
        param.sendStartTime > param.validStartTime ||
        param.sendEndTime > param.validEndTime
      ) {
        ...
        if (this.afterMerchant.length <= 0) {
          ...
          if (formObj.shopId == "按商场可用") {
            ...
            if (count <= 0) {
              ...
              param.supplierId = merchantarr.join("-");
            }
          } else {
            this.ispass4 = true;
      ...
      if (
        ...
        ApiSetting.editCoupon.data = param;
        this.$http(ApiSetting.editCoupon).then(msg => {
